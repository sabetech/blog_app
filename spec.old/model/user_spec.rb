...
RSpec.describe User, type: :model do
  ...
  it 'Posts Counter attribute should be an integer number' do
    subject.posts_count = 'some random string'
    expect(subject).to_not be_valid
  end
  ...
  it 'last_three_posts method should return the last three posts' do
    ...
    expect(User.three_most_posts(user.id).size).to be(3)
    expect(User.three_most_posts(user.id).pluck(:id)).to match_array(user.posts.last(3).pluck(:id))
  end
end
